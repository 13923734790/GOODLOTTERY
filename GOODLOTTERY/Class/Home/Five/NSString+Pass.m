...
-(BOOL)isWeixinNumber
{
    NSString *weixinRegex = @"^[A-Za-z0-9]+$";
    //NSString *weixinRegex = @"[A-Za-z0-9]{0,}";
    NSPredicate *weixinTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", weixinRegex];
    return [weixinTest evaluateWithObject:self];
}

-(BOOL)isStationaryNumber
{
    NSString *StationaryRegex = @"^[0-9]{3,5}-[0-9]{7,8}$";
    NSPredicate *StationaryTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", StationaryRegex];
    return [StationaryTest evaluateWithObject:self];
}
...
{
  CGRect rect = [self boundingRectWithSize:LableSize options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesFontLeading  |NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: font}                                       context:nil];
    return rect.size;
}
@end
